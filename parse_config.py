...
class ConfigParser:
    ...
    def from_args(cls, args, options=''):
        """
        调用此方法，解析命令行界面传进来的参数，如resume信息，路径信息等，在train.py文件中调用
        ...
        if args.device is not None:     # 检测是否多卡计算
            os.environ["CUDA_VISIBLE_DEVICES"] = args.device
        if args.resume is not None:     # 检测是否从断点继续
        ...
        return cls(config, resume, modification)    # cls类本身
    ...
    def get_logger(self, name, verbosity=2):
        ...
        logger = logging.getLogger(name)    # log实例化
        logger.setLevel(self.log_levels[verbosity])    # 设置levels
        return logger
